...
var prompt = "Enter a radius and an angle (in degrees), e.g., 8.9 120, " + " or %s to quit."
...
func interact(questions chan polar, answers chan cartesian) {
	...
	for {
		...
		if _, err := fmt.Sscanf(line, "%f %f", &radius, &θ); err != err {
			fmt.Fprintln(os.Stderr, "incalid input")
			continue
		}
